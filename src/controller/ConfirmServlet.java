		...
		NumberFormat nf = NumberFormat.getInstance();
		 Date dNow = new Date( );
	      ...
	      new SimpleDateFormat ("dd/MM/yyyy 'at' hh:mm:ss a");
		   ...
		   String text ="<strong>hàng là - "+ username +" - </strong><i> "+ ft.format(dNow) +"</i> <ul>";
		   if(cart!=null){
			   for(Cart c : cart){
				   ...
				   text+="<li>"+ productDAO.getProduct(c.getP().getMa_san_pham()).getTen_san_pham()+": "+ nf.format(productDAO.getProduct(
							c.getP().getMa_san_pham()).getGia_ban()) +"VND </li>";
				   resuft=(int)total;
				   ...
				   User p  = new User(u.getUser_id(),username,u.getPassword(), u.getNgaysinh(), u.getGioitinh(), u.getEmail(), u.getSdt(), u.getDiachi(), "2",2000);
				   userDAO.updateUser(p);
			   }
		   }
		   text+="tong thanh toan la: <strong>"+ nf.format(total) +" VND </strong>";
		   ...
		   response.sendRedirect("/web_security06/index.jsp");
	}
