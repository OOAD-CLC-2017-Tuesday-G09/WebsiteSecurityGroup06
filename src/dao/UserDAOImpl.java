...
//		System.out.println(dao.checkUser("admin"));
	...
	public void updateUser(User u) {
		Connection con = DBConnect.getConnection();
		String sql = "update user set user_id=?, password=?, ngaysinh=?, gioitinh=?, email=?, sdt=?, diachi=?, role=?,sodutaikhoan=? where username=?";
		try {
			...
			ps.setInt(10, u.getSodutaikhoan());
			ps.executeUpdate();
			con.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	@Override
	public void updateSodu(User u) {
		Connection con = DBConnect.getConnection();
		String sql = "update user set sodutaikhoan=? where username=?";
		try {
			PreparedStatement ps = (PreparedStatement) con
					.prepareStatement(sql);
			ps.setInt(10, u.getSodutaikhoan());
	...
	public User getUser(String name) {
		...
		try {
			...
			while (rs.next()) {
				...
				int sodutaikhoan= rs.getInt("sodutaikhoan");
				u = new User(user_id, username, password, ngaysinh, gioitinh, email, sdt, diachi, role,sodutaikhoan);
			}
			con.close();
		} catch (SQLException e) {
			e.printStackTrace();
